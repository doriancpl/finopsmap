...
<div id="topbar">
  ...
  <div id="topbarActions" style="display:flex;gap:14px;align-items:center;flex:1;justify-content:flex-end">
    ...
    <button class="veille-btn" id="heatmapBtn" onclick="setHeatmap()"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:5px;position:relative;top:-1px"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg><span class="tb-label">Heatmap</span></button>
...
<div id="filters">
  ...
  <div id="discountBtn" onclick="toggleDiscount()" onmouseenter="if(!discount){document.getElementById('discountLbl').style.color='var(--accent)'}" onmouseleave="if(!discount){document.getElementById('discountLbl').style.color='#c8d0e8'}" style="display:inline-flex;align-items:center;background:transparent;border:1px solid var(--b1);border-radius:6px;overflow:hidden;cursor:pointer;transition:border-color .2s,color .2s">
    <span id="discountLbl" style="padding:8px 15px;color:#c8d0e8;font-family:'Syne',sans-serif;font-size:.9rem;font-weight:700;white-space:nowrap;transition:color .2s;display:flex;align-items:center;gap:7px">EDP %</span>
...
function setView(v) {
  ...
  if (famSel) {
    if (isRds) {
      ...
      famSel.innerHTML = '<option value="all">All</option><option value="general">General</option><option value="burstable">Burstable</option><option value="compute">CPU</option><option value="memory">Memory</option><option value="storage">Storage</option><option value="gpu">GPU</option><option value="other">Other</option>';
    }
    famSel.value = 'all';
  }
...
function matchNum(val, filter) {
  ...
  const m = f.match(/^([<>]=?)?([0-9]+[.]?[0-9]*)$/);
  if (!m) return String(val).includes(f);
...
function openSeriesModal(iname, fam) {
  ...
  // Left: specs
  let specsHtml = '<table class="id-card">';
  specsHtml += cardRow('Instance Type', '<span style="color:'+color+';font-weight:700">'+cur.iname+'</span>');
  specsHtml += cardRow('Instance Family', famLabel);
  if (hasProc) specsHtml += cardRow('Processor', cur.processor);
  specsHtml += cardRow('vCPUs', cur.vcpu);
  specsHtml += cardRow('Memory', ramFmt(cur.ram));
  if (hasStor) specsHtml += cardRow('Storage', cur.storage);
  if (cur.network) specsHtml += cardRow('Network', cur.network);
  specsHtml += '</table>';

  // Right: pricing
  const od = cur.price;
  function priceFmt(v) { if (!v) return null; return f4(v * (periodMult[period]||1)) + '\u202f' + currSym(); }
  function savBadge(v) { if (!v || !od) return ''; const pct = Math.round((1 - v/od)*100); if (pct <= 0) return ''; return '<span class="modal-sav-badge">-'+pct+'%</span>'; }
  function priceRow(label, v, highlight) {
    if (!v) return '';
    const valStyle = highlight ? 'color:'+color+';font-size:.92rem' : '';
    const lblStyle = highlight ? 'color:#dde3f0;font-weight:700' : '';
    return '<div class="modal-price-row'+(highlight?' modal-price-highlight':'')+'"><span class="modal-price-label" style="'+lblStyle+'">'+label+'</span><span class="modal-price-val" style="'+valStyle+'">'+priceFmt(v)+savBadge(v)+'</span></div>';
  }
  function priceSection(title, rows) {
    const content = rows.join('');
    if (!content) return '';
    return '<div class="modal-price-section"><div class="modal-price-section-title">'+title+'</div>'+content+'</div>';
  }
  const isAwsView = view === 'aws' || view === 'rds';
  const isAzureModal = view === 'azure' || view === 'psql';
  let pricingHtml = '<div class="modal-pricing">';
  pricingHtml += priceSection('On-Demand', [priceRow('On-Demand', od, true)]);
  if (isAwsView || isAzureModal) {
    pricingHtml += priceSection('Reserved Instances', [
      priceRow('1Y · No Upfront',  cur.price_1yr),
      priceRow('1Y · All Upfront', cur.price_1yr_all),
      priceRow('3Y · No Upfront',  cur.price_3yr),
      priceRow('3Y · All Upfront', cur.price_3yr_all),
    ]);
  }
  if (isAwsView && (cur.sp_1yr_no || cur.sp_1yr_all || cur.sp_3yr_no || cur.sp_3yr_all)) {
    pricingHtml += priceSection('Compute Savings Plan', [
      priceRow('1Y · No Upfront',  cur.sp_1yr_no),
      priceRow('1Y · All Upfront', cur.sp_1yr_all),
      priceRow('3Y · No Upfront',  cur.sp_3yr_no),
      priceRow('3Y · All Upfront', cur.sp_3yr_all),
    ]);
  }
  if (cur.spot) {
    pricingHtml += priceSection('Spot', [priceRow('Spot (avg)', cur.spot)]);
  }
  pricingHtml += '</div>';

  const cardHtml = '<div class="modal-card-layout">'+specsHtml+pricingHtml+'</div>';
